...
```{yaml}
#| filename: "test R .github/workflows/ci.ym`"
name: CI
...
You know the drill! Modify your workflow to style and lint your code,
...
now if you're code is already lint-free.
::::

:::{.callout-caution}
# Are your tests failing?
If you have some failing tests, the workflow will fail before it gets to the
lint & style step. You can temporarily comment-out the test step with hashes 
(`#`) so you can continue through this tutorial, but _don't forget_ to uncomment
these lines later!
:::
...
Generally the higher the code coverage, the better. However, a code coverage of
...
#### You'll need to make sure the test suite generates a report that codecov can ingest.
...
it to codecov all with one function. You won't need codecov's 
...
If your tests aren't passing, codecov won't be able to generate a report.

![](/img/codecov-report.png)

:::{.callout-tip}
# codecov setup
**React to the Slack message with ✅ once your CI workflow completes
...
At <https://codecov.io>, go to your project, click `Settings`, and select
`Badges & Graphs`. Under the 'Markdown' heading, copy the chunk, then paste it
into your README file right after your CI badge.
...
We have done all of this work in one workflow, but you can have multiple
